...
export function PremiumButton({
  ...
  const baseClasses = 'relative overflow-hidden rounded-xl font-semibold transition-all duration-300 hover-lift';
  
  const variantClasses = {
    primary: 'bg-gradient-premium text-white hover-glow',
    secondary: 'glass-base text-gray-800 dark:text-white hover:bg-white/20',
    ghost: 'bg-transparent text-gray-700 dark:text-gray-300 hover:bg-white/10',
    gradient: 'bg-gradient-aurora animate-gradient text-white hover-glow'
  };
  ...
  return (
    <div
      className={cn(
        ...
        animated && 'hover-lift animate-scale-in',
        className
      )}
    >
      {children}
...
export function FloatingOrbs() {
  const [orbs, setOrbs] = useState<Array<{id: number, x: number, y: number, delay: number}>>([]);
  ...
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden -z-10">
      {orbs.map((orb) => (
        <div
          key={orb.id}
          className="absolute w-64 h-64 rounded-full bg-gradient-aurora opacity-20 animate-float blur-xl"
          style={{
            left: `${orb.x}%`,
            top: `${orb.y}%`,
    ...
    <div className="space-y-2">
      ...
      <div className="relative">
        <input
          ...
          disabled={disabled}
          className={cn(
            'w-full px-4 py-3 rounded-xl glass-base',
...
interface GradientTextProps {
  children: React.ReactNode;
  gradient?: 'premium' | 'aurora' | 'cosmic' | 'sunset';
  className?: string;
  animated?: boolean;
}
  ...
  if (variant === 'gradient') {
    return (
      <div className={cn('relative', sizeClasses[size], className)}>
        <div className="absolute inset-0 rounded-full bg-gradient-aurora animate-spin" />
...
export function Notification({
  ...
  return (
    <div
      className={cn(
        'glass-base rounded-xl p-4 border transition-all duration-300',
        config.bg,
        config.border,
